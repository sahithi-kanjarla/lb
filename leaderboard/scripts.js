...
function populateLeaderboard(data) {
    ...
    data.forEach((item, index) => {
        if (parseInt(item['# of Skill Badges Completed']) === 15) badgesCompletedCount++;
        if (parseInt(item['# of Arcade Games Completed']) >= 1) gamesCompletedCount++;

        let row = `<tr>
            <td>${index + 1}</td>
            <td>${item['User Name'] || 'N/A'}</td>
            <td>${item['Access Code Redemption Status'] || 'N/A'}</td>
            <td>${parseInt(item['# of Skill Badges Completed']) + parseInt(item['# of Arcade Games Completed']) || 0}</td>
            <td>${item['# of Skill Badges Completed'] || 0}</td>
            <td>${item['# of Arcade Games Completed'] || 0}</td>
            <td><img src="${item['Special Badge'] || 'default-badge.png'}" class="special-badge" alt="Badge"></td>
        </tr>`;
